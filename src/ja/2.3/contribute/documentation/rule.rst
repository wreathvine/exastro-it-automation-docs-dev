...
| ユーザーにとって価値の高い文章とは、最小のコストでユーザーの目的に到達するための手段が明確かつ簡潔に記述された文章です。ユーザーにとって価値の高い文章を書くためには、文章の目的・目標(ゴール)の共有をし、ゴールまでの手段(プロセス)を明確にする必要があります。また、プロセスが発散するのを防ぐために前提(スタート)を設定することも必要です。
...
| 料理をする上で包丁の刃渡りや鍋の容量はそれほど大きな問題ではなく、本当にユーザーが関心を持っていることは、目的の料理を完成させる方法です。

| ユーザーにとって価値の高い文章の構成の具体的な構成は、下記のようになっているべきです。
...
| 「1. 目的」は、読み手(ユーザー)との視点を合わせるために記述するべき内容です。
| なぜその操作が必要なのかという理由や、何のための作業なのかといった目的を明確にすることで、ユーザーが同じ視点で文章を読み始めることができるため、よりスムーズに理解ができます。また、探している内容がその文章に含まれているかどうかを判断する材料にもなります。
...
|  e.g. 
|  　ビルド、生成  --> 「ビルド」に統一します。
|  　できます、出来ます  --> 「できます」に統一します。


ターミノロジー
==============

| ターミノロジー（専門用語）を用いる場合は、必ずその意味を最初に説明してから使用します。


文法エラー
==========

| エディタに付属される構文チェッカーやビルド時に出力結果に Warning や Error が含まれてはいけません。
