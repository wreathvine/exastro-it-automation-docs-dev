...
パスワードを忘れた場合
----------------------
| 下記の手順で、パスワードをリセットすることが出来ます。

前提条件
^^^^^^^^^^^^^^^^^^^^^^^

| メール送信サーバー設定が登録されている必要があります。
| パスワードを忘れたユーザーのemailが設定されている必要があります。

.. tip:: 
   | パスワードを忘れたユーザーのemailが設定されていないときは、オーガナイゼーション管理者にパスワードの変更を依頼してください。

パスワードのリセット
^^^^^^^^^^^^^^^^^^^^^^^

#. | ログイン画面の :guilabel:`パスワードをお忘れですか？` をクリックします。

   .. figure:: /images/ja/manuals/platform/login/password_change_login_display.png
      :width: 600px
      :align: left

#. | パスワードをリセットしたいユーザーのユーザー名またはメールアドレスを入力し、 :guilabel:`送信` をクリックします。

   .. figure:: /images/ja/manuals/platform/login/password_change_user.png
      :width: 600px
      :align: left

#. | 対象のメールアドレスに パスワードのリセット のメールが届きますので、:guilabel:`パスワードのリセット` をクリックします。

   .. figure:: /images/ja/manuals/platform/login/password_change_mail.png
      :width: 600px
      :align: left

#. | 新しいパスワードを入力し、 :guilabel:`送信` をクリックすると、パスワードが更新されてWorkspace管理画面が表示されます。

   .. figure:: /images/ja/manuals/platform/login/password_change.png
      :width: 600px
      :align: left