...
-  **作業フロー詳細と参照先**
   ...
   #. | **パラメータシートの作成**
      | パラメータシート作成コンソールのパラメータシート定義・作成の画面から、作業対象サーバの設定に使用するデータを登録するためのメニューを作成します。
...
#. | パラメータシート作成機能で作成したパラメータシートの項目の設定値とMovementの変数を紐付けます。登録した情報は内部の処理により代入値管理と作業対象ホストに反映されます。
...
.. note:: | **ファイル埋込変数とテンプレート埋込変数をPlaybookの変数に紐付して使用する例**
   ...
   #. | :menuselection:`パラメータシート作成-->パラメータシート定義・作成` で「Ansible共通:ファイル管理:ファイル埋込変数名」「Ansible共通:テンプレート管理:テンプレート埋込変数名」をパラメータシートの項目としてパラメータシート作成後、 :menuselection:`入力用-->サンプルパラメータシート(メニュー名)` で項目の設定値としてファイル埋込変数とテンプレート埋込変数を登録します。
...
.. #. | パラメータシート(バンドル)
..    | パラメータシート(バンドル)を使用する際に、リピート設定されている項目において、まとまり単位で値が空の場合、代入値自動登録設定でNULL連携を有効にしていても、代入値管理に登録はされません。
