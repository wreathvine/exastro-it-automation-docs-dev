...
.. table:: ラベル付与におけるユースケースについて
 ...
 |                                                 |                |                | ≠（不一致）のみ|             |             | ≠ → 〇 のみ  |
 +-------------------------------------------------+----------------+----------------+----------------+-------------+-------------+--------------+
 | 検索条件に正規表現を使用したい                  | 〇             | その他 のみ    | RegExp,        | ー          | 〇          | 〇,ー        |
 |                                                 |                |                |                |             |             |              |
 |                                                 |                |                | RegExp\        |             |             |              |
 |                                                 |                |                | (DOTALL),      |             |             |              |
 |                                                 |                |                |                |             |             |              |
...
.. list-table:: ラベル付与における値のデータ型について
   ...
   * - 空判定
     ...
     - | 空文字、[]、{}、0、Falseのみ
       | 例　""
   * - その他
     - RegExp(DOTALL), RegExp(MULTILINE)のみ
     - ー
...
.. table:: 正規表現の種類について
 ...
 | **比較方法**          | **説明**                                                         | メール本文          | 結果         | 
 +=======================+==================================================================+=====================+==============+
 | RegExp                | オプションなしで正規表現を行います。                             | 対象                | web01        |
 |                       |                                                                  |                     |              |
 |                       |                                                                  | サーバ:web01.com    |              |
 +-----------------------+------------------------------------------------------------------+---------------------+--------------+
 | RegExp(DOTALL)        | 「.」を改行を含む全ての文字にマッチさせることができます。        | ・・・（文章）      | 障害: web01  |
 |                       |                                                                  |                     |              |
 |                       | このオプションがなければ、改行を含まない全ての文字、になります。 | 対象                |              |
 |                       |                                                                  |                     |              |
 |                       |                                                                  | サーバ:web01.com    |              |
 |                       |                                                                  |                     |              |
 |                       |                                                                  | ・・・（文章）      |              |
 +-----------------------+------------------------------------------------------------------+---------------------+--------------+
 | RegExp(MULTILINE)     | 「^」「$」が各行の先頭と末尾にマッチさせることができます。       | サーバ:web01.com    | web01        |
 |                       |                                                                  |                     |              |
 |                       |                                                                  | 障害が発生しました。|              |
 +-----------------------+------------------------------------------------------------------+---------------------+--------------+
